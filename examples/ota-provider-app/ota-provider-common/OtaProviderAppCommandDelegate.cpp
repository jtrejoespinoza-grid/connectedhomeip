...
{
    //mCameraDevice->HandleSimulatedZoneTriggeredEvent();
    return "This is the response from the command";
}
...
void OtaProviderAppCommandHandler::HandleCommand(intptr_t context)
{
    ...
    if (name == "GetQueryImageResponse")
    {
        ...
        out["Name"]     = "GetQueryImageResponse";
        out["Cluster"]  = cluster;
        out["Endpoint"] = endpoint;
        std::string payload =  self->GetQueryImageResponse();
        out["Payload"] = payload;
